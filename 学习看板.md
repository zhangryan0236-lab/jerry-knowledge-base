...
@media(max-width:1100px){.dsh .g4,.dsh .g4b{grid-template-columns:1fr 1fr}.dsh .mets{grid-template-columns:1fr 1fr}}
...
> 💡 **数据说明**：所有数据存储在浏览器 localStorage 中（与 Obsidian 绑定）。清空浏览器数据会导致数据丢失！
> 🔧 **打开后不渲染？** 确保已开启 `Dataview` 插件，然后切到**阅读模式**（`Ctrl/Cmd + E`），或者直接切到阅读视图。
